...
export function selectInterruption(game: GameData, random = Math.random) {
  const weighted = interruptions
    .filter(event => event.id !== 'commute' && matches(game, event.conditions) && game.minutes >= (game.cooldowns[event.id] ?? 0))
    .map(event => ({event, weight:event.probability + (game.futureChance[event.npc] ?? 0) + Math.max(0,game.relations[event.npc] ?? 0)/1000}))
...
export function selectWorldEvent(game: GameData, random = Math.random) {
  return randomEvents.find(event => matches(game,event.conditions) && game.minutes >= (game.cooldowns[event.id] ?? 0) && random() < event.probability) ?? null
}
