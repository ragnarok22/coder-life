...
export function cylinderFor(topRatio: number) {
  let geometry = cylinders.get(topRatio);
  if (!geometry) {
    geometry = new CylinderGeometry(topRatio, 1, 1, 10);
    cylinders.set(topRatio, geometry);
  }
  return geometry;
}
